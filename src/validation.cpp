...
#if defined(NDEBUG)
# error "Criptoreal Core cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in reals) are considered zero fee (for relaying, mining and transaction creation) */
...
CAmount GetBlockSubsidy(int nPrevBits, int nHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
  int halvings = nHeight / consensusParams.nSubsidyHalvingInterval;
    ...
    if (halvings >= 35)
        return 0;
          ...
          if (nHeight == 1)
              ...
              return 150000 * COIN; // giveaway fund, faucets, dice and what not - reduced from 175k on old chain
          else if (nHeight == 9)
    ...
    // LogPrintf("height %u diff %4.2f reward %d\n", nPrevHeight, dDiff, nSubsidyBase);
      ...
      // Subsidy is cut in half every 825,000 blocks which will occur approximately every 3 years.
    nSubsidy >>= halvings;
...
CAmount GetMasternodePayment(int nHeight, CAmount blockValue)
{
    ...
    int nMNPIPeriod = Params().GetConsensus().nMasternodePaymentsIncreasePeriod;
                                                                      ...
                                                                      // mainnet:
    if(nHeight > nMNPIBlock)                  ret += blockValue / 10; // 20 + 10 = 30% to start
    if(nHeight > nMNPIBlock+(nMNPIPeriod* 1)) ret += blockValue / 10; // 30 + 10 = 40 % after 720*7 - about a week
    if(nHeight > nMNPIBlock+(nMNPIPeriod* 2)) ret += blockValue / 10; // 40 + 10 = 50 % after (720*7)*2 - about two weeks
...
void ThreadScriptCheck() {
    RenameThread("criptoreal-scriptch");
...
static bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck = false)
{
    ...
    // CRS : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(CRS): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(CRS): couldn't find masternode or superblock payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // CRS : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(CRS): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(CRS): spork is off, skipping transaction locking checks\n");
    }
