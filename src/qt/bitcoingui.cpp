    ...
    platformStyle(platformStyle)
{
    ...
    QString windowTitle = tr("Criptoreal Core") + " - ";
#ifdef ENABLE_WALLET
    ...
    {
        progressBar->setStyleSheet("QProgressBar { background-color: #F8F8F8; border: 1px solid grey; border-radius: 7px; padding: 1px; text-align: center; } QProgressBar::chunk { background: QLinearGradient(x1: 0, y1: 0, x2: 1, y2: 0, stop: 0 #5DA14E, stop: 1 #5DA14E); border-radius: 7px; margin: 0px; }");
    }
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Criptoreal address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and criptoreal: URIs)"));
    ...
    aboutAction = new QAction(QIcon(":/icons/" + theme + "/about"), tr("&About Criptoreal Core"), this);
    aboutAction->setStatusTip(tr("Show information about Criptoreal Core"));
    ...
    optionsAction->setStatusTip(tr("Modify configuration options for Criptoreal Core"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Criptoreal addresses to prove you own them"));
    verifyMessageAction = new QAction(QIcon(":/icons/" + theme + "/transaction_0"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Criptoreal addresses"));
    ...
    openAction->setStatusTip(tr("Open a criptoreal: URI or payment request"));

    showHelpMessageAction = new QAction(QApplication::style()->standardIcon(QStyle::SP_MessageBoxInformation), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the Criptoreal Core help message to get a list with possible Criptoreal Core command-line options"));
...
void BitcoinGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
    ...
    QString toolTip = tr("Criptoreal Core client") + " " + networkStyle->getTitleAddText();
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        labelConnectionsIcon->setToolTip(tr("%n active connection(s) to Criptoreal network", "", count));
    } else {
...
{
    QString strTitle = tr("Criptoreal Core"); // default title
    ...
    // Append title to "Criptoreal Core - "
    if (!msgType.isEmpty())
        strTitle += " - " + msgType;
