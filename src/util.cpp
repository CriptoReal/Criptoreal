...
const char * const BITCOIN_CONF_FILENAME = "criptoreal.conf";
const char * const BITCOIN_PID_FILENAME = "criptoreald.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "criptoreal" is a composite category enabling all Criptoreal-related debug output
            if(ptrCategory->count(string("criptoreal"))) {
...
#else
    const char* pszModule = "criptoreal";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Criptoreal
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Criptoreal
    // Mac: ~/Library/Application Support/Criptoreal
    // Unix: ~/.criptoreal
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Criptoreal";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Criptoreal";
#else
    // Unix
    return pathRet / ".criptoreal";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty criptoreal.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override criptoreal.conf
